...
my $input = "nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
...
foreach my $line (split /\n/, $input) {
  ...
  foreach my $char (split //,$line){
    if(exists $occurrence{$char}){
      $occurrence{$char}+=1;
    }else{
        $occurrence{$char} = 1;
    }
  }
  my @sorted = sort{$occurrence{$b} <=> $occurrence{$a}|| ($a cmp $b)} keys %occurrence;
  foreach my $res (@sorted) {
    if($res =~ /[a-z]/){
      $checksumCheck = $checksumCheck.$res;
    }
  }
  my $checksumCheckSub = substr($checksumCheck,0,5);

  if($checksum eq $checksumCheckSub){
      #print "$line - Sector ID - $sectorID\n";
      $line =~ /(^[a-z]+.+)-[0-9]+/g;
      print rotateString($1,$sectorID);
      print "-$sectorID\n========\n";
  }
...
sub rotateString {
  ...
  foreach my $char (split //, $string) {
    ...
    my $letterEncrypt = ($alphaNums{$char} + $number) % 26;
    $returnString = $returnString."$numAlphas{$letterEncrypt}";
