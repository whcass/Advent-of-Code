...
my $input = "/dev/grid/node-x0-y0     94T   65T    29T   69%
...
foreach my $node (split /\n/,$input) {
  $node =~ /\/dev\/grid\/node-x(\d+)-y(\d+)\s+(\d+)T\s+(\d+)T\s+(\d+)T/g;
  push @nodeData, {
    x=>$1,
    y=>$2,
    size=>$3,
...
my $matched = 0;
my $unmatched = $#nodeData;
foreach my $node1 (@nodeData) {
  #print "$matched-$unmatched\n";
  foreach my $node2 (@nodeData) {
      ...
      if($node1->{used}!=0){
        if(($node1->{x}!=$node2->{x})&&($node1->{y}!=$node2->{y})){
          if($node1->{used}<$node2->{avail}){
            #$node1->{used}=0;
            #$node1->{match}=0;
            #$node2->{used} = $node1->{used} + $node2->{used};
            #$node2->{avail} = $node2->{avail}-$node1->{used};
            #print "match\n";
            $matched++;
          }
        }
